...
template<typename T>
void Tensor<T>::copy_from(Tensor<T>& from)
{
    CHECK(from.size() == _size);
    CHECK(from.is_contiguous());
    CHECK(is_contiguous());

    if(_device < 0 && from.device() < 0){
        CHECK(memcpy(mutable_cpu_data(), from.mutable_cpu_data(), sizeof(T) * _size.count()));
    }
    else if(_device < 0 && from.device() >= 0){
        CUDA_CHECK(cudaMemcpy(mutable_cpu_data(),
                from.mutable_gpu_data(),
                sizeof(T) * _size.count(),
                cudaMemcpyDeviceToHost));
    }else if(_device >= 0 && from.device() < 0){
        CUDA_CHECK(cudaMemcpy(mutable_gpu_data(),
                from.mutable_cpu_data(),
                sizeof(T) * _size.count(),
